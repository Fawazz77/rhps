...
ballancee = Redis:get(TheRhp.."nool:flotysb"..msg.sender.user_id) or 0
if tonumber(coniss) < 199 then
return LuaTele.sendText(msg.chat_id,msg.id, "⇜ الحد الادنى للمتاجره المسموح هو 200 دينار 💸\n༄","md",true)
end
if tonumber(ballancee) < tonumber(coniss) then
return LuaTele.sendText(msg.chat_id,msg.id, "⇜ فلوسك ماتكفي \n༄","md",true)
end
...
local Descriptioontt = modarba[math.random(#modarba)]
local modarbaa = math.random(1,90);
elseif Descriptioontt == "1" or Descriptioontt == "2" then
ballanceekku = math.floor(coniss / 100 * modarbaa)
ballanceekkku = math.floor(ballancee + ballanceekku)
Redis:incrby(TheRhp.."nool:flotysb"..msg.sender.user_id , math.floor(ballanceekku))
Redis:setex(TheRhp.."nnoooo" .. msg.sender.user_id,1200, true)
LuaTele.sendText(msg.chat_id,msg.id, "⇜ مُتاجره ناجحة \n⇜ نسبة الربح ↢ "..modarbaa.."%\n⇜ المبلغ الذي ربحته ↢ ⦗ "..ballanceekku.." دينار 💸 ⦘\n⇜ فلوسك صارت ↢ ⦗ "..ballanceekkku.." دينار 💸 ⦘\n༄","md",true)
end
else
LuaTele.sendText(msg.chat_id,msg.id, "⇜ ماعندك حساب بنكي\n⇜ اكتب ( `انشاء حساب بنكي` )","md",true)
end
...
if Redis:sismember(TeRhp."noooybgy",msg.sender.user_id) then
