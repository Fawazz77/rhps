...
if data.new_chat_member.member_id.user_id == tonumber(FDFGERB) or data.old_chat_member.member_id.user_id == tonumber(FDFGERB) then
...
--code start
if Redis:sismember(FDFGERB..'Black:listBan:',chat_id) then ---check if ban
send(chat_id,0,"\n- هذه القروب محظور سوف اغادر","md",true)  
bot.leaveChat(chat_id)
end ---end check if ban
local Info_Chats = LuaTele.getSupergroupFullInfo(chat_id) ---check if count is true
if tonumber(Info_Chats.member_count) < tonumber((Redis:get(FDFGERB..'FDFGERB:Num:Add:Bot') or 0)) then
LuaTele.sendText(chat_id,0,'• عدد الاعضاء قليل لا يمكن تفعيل القروب\n يجب ان يكون اكثر من '..Redis:get(FDFGERB..'FDFGERB:Num:Add:Bot'),"md",true)
bot.leaveChat(chat_id)
end---end check if count is true
if not Redis:get(FRFGERB.."FDFGERB:BotFree") then
return LuaTele.sendText(chat_id,0,"\n⇜ الوضع الخدمي تم تعطيله من قبل مطور البوت ","md",true)  
end
if not Redis:sismember(FDFGERB.."FDFGERB:ChekBotAdd", chat_id) then ---done active
local Get_Chat = LuaTele.getChat(chat_id)
local Info_Chats = LuaTele.getSupergroupFullInfo(chat_id)
local UserInfo = LuaTele.getUser(who_promot)
----
...
-----
Redis:set(FDFGERB.."FDFGERB:Status:Link"..chat_id,true) 
Redis:set(FDFGERB.."FDFGERB:Status:Games"..chat_id,true) 
Redis:set(FDFGERB.."FDFGERB:Lock:edit"..chat_id,true) 
Redis:sadd(FDFGERB.."FDFGERB:ChekBotAdd",chat_id)
Redis:set(FDFGERB.."FDFGERB:Status:Id"..chat_id,true)
Redis:set(FDFGERB.."FDFGERB:Status:Reply"..chat_id,true)
Redis:set(FDFGERB.."FDFGERB:Status:ReplySudo"..chat_id,true)
Redis:set(FDFGERB.."FDFGERB:Status:BanId"..chat_id,true)
Redis:set(FDFGERB.."FDFGERB:Status:SetId"..chat_id,true) 
-----
local Info_Members = LuaTele.getSupergroupMembers(chat_id, "Administrators", "*", 0, 200)
local List_Members = Info_Members.members
for k, v in pairs(List_Members) do
if Info_Members.members[k].bot_info == nil then
if Info_Members.members[k].status.luatele == "chatMemberStatusCreator" then
Redis:sadd(FDFGERB.."FDFGERB:TheBasicsQ:Group"..chat_id ,v.member_id.user_id) 
else
Redis:sadd(FDFGERB.."FDFGERB:Addictive:Group"..chat_id ,v.member_id.user_id) 
end
end
end
-----
local txt = '「 ['..UserInfo.first_name..'](tg://user?id='..who_promot..')⁪⁬‌‌‌‌ 」\n⇜ تم تفعيل المجموعة ['..Get_Chat.title..']('..Info_Chats.invite_link.invite_link..') تلقائياً\n\n⇜ ارسل الاوامر لعرض اوامر البوت'
local reply_markup = LuaTele.replyMarkup{
type = 'inline',
data = {
{
{text = 'الحماية',data =IdUser..'/'.. 'NoNextSeting'}, 
},
}
}
return LuaTele.sendText(chat_id, 0 , txt, 'md', true , false, false, false, reply_markup)
end ---end done active
--code end
end
end
